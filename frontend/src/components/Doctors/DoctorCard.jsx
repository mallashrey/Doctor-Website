...
const DoctorCard = ({doctor}) => {
  ...
  return (
    <div className='p-3 lg:p-5'>
        <div>
            {/* To add images of the doctors */}
            <img src={photo} className='w-full' alt="" />
        </div>
        
        {/* TO add names to the images */}
        <h2 className='text-[18px] leading-[30px] lg:text-[26px] lg:leading-9 
        text-headingColor font-[700] mt-3 lg:mt-5 '>{name}</h2>


    {/* To create the specialization tag colourful beneath the individual doctors' photos */}
    <div className="mt-2 lg:mt-4 flex items-center justify-between "> 
        ...
        {/* Adds starIcon below the picture and also shows the avgRating and total ratings*/}
        <div className="flex items-center gap-[6px] ">
            ...
            <span className='text-[14px] leading-6 lg:text-[16px] lg:leading-7 font-[400] text-textColor '>
                ({totalRating})
            </span> {/*because all the elements are in the same line, we use span*/}
        </div>
    </div>   
        {/* add totalpatients */}
        <div className="mt-[18px] lg:mt-5 flex items-center justify-between ">
            ...
            {/* Add Link Arrow*/}
            <Link to={`/doctors/${doctor.id}`} className='w-[44px] h-[44px] rounded-full border border-solid
            border-[#181A1E] flex items-center justify-center group hover:bg-primaryColor
            hover:border-none'><BsArrowRight className="group-hover:text-white w-6 h-5" />
            </Link>   
